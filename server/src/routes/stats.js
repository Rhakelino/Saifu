...
router.get('/', async (req, res) => {
    try {
        ...
        // Get total balance (net worth) from all wallets
        const walletsData = await db
            .select({
                totalBalance: sql`SUM(CAST(${wallets.balance} AS DECIMAL))`,
            })
            .from(wallets)
        ...
        if (startDate) {
            dateConditions.push(gte(transactions.transactionDate, startDate));
        }
        if (endDate) {
            dateConditions.push(lte(transactions.transactionDate, endDate));
        }
        ...
        // Process monthly stats into structured format
        const monthlyMap = {};
